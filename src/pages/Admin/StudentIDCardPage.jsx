...
const StudentIDCardPage = () => {
    ...
    const fetchStudentsByClass = async (classId) => {
        try {
            ...
            const students = res.data.data?.results || [];
            const selectedClassObj = classes.find((c) => c.id === classId);
            const filtered = students.filter((s) =>
                s.class_name?.includes(selectedClassObj?.class_name)
            );
    ...
    const printSingleCard = async (student) => {
        ...
        setSingleProgress(0);
        try {
            const card = document.getElementById(`card-${student.profile_id}`);
            const front = card.children[0];
            const back = card.children[1];
            ...
            const fileName = `id-card-${student.first_name}-${student.last_name}.pdf`;
            doc.save(fileName);
        ...
        <div className="p-6 bg-blue-50 min-h-screen">
                        ...
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <div className="w-[300px] h-[175px] relative bg-white rounded-lg overflow-hidden shadow border border-blue-900">
                                ...
                                <div className="flex p-2 gap-3">
                                    <img
                                        src={selectedStudentCard.profile_picture ? `${API}${selectedStudentCard.profile_picture}` : "/default-avatar.png"}
                                        alt="student"
                                        className="w-[60px] h-[70px] rounded border object-cover"
                                    />
                                    <div className="text-[11px] text-gray-700 space-y-[2px]">
                                        <h2 className="text-sm font-bold text-blue-900">STUDENT ID CARD</h2>
                                    ...
                                    <div className="relative w-[280px] top-[135px] px-7">
                                        <Barcode
                                            value={JSON.stringify({
                                                name: `${selectedStudentCard.first_name} ${selectedStudentCard.last_name}`,
                                                student_id: selectedStudentCard.profile_id,
                                                class: selectedStudentCard.class_name,
                            ...
                            <div className="w-[300px] h-[175px] bg-white rounded-lg overflow-hidden shadow border border-blue-900 text-[10px] text-gray-700 relative">
                                ...
                                <div className="absolute bottom-1 left-0 w-full px-3 flex justify-between text-[9px] text-blue-900">
                                    <span>📞 {selectedStudentCard.phone || '+92-000-0000000'}</span>
                                    <span>🌐 {selectedStudentCard.email || 'school@example.com'}</span>
                                    <span>📍 {selectedStudentCard.address || 'Kotli, AJK'}</span>
                                </div>
                            </div>
            ...
            <div className="flex justify-between items-center bg-blue-900 text-white px-6 py-3 rounded-t-md shadow">
                ...
                <div className="flex gap-3">
                    <Select
                        options={classes}
                        getOptionLabel={(cls) => `${cls.class_name} ${cls.section || ''}`}
                        getOptionValue={(cls) => cls.id}
                        onChange={setSelectedClass}
                        placeholder="Select Class"
                        className="text-black min-w-[200px]"
                        ...
                        <div className="w-[300px] h-[175px] relative bg-white rounded-lg overflow-hidden shadow border border-blue-900 print:block">
                            ...
                            <div className="flex p-2 gap-3">
                                ...
                                <div className="text-[11px] text-gray-700 space-y-[2px]">
                                    ...
                                    <p><strong>Class:</strong> {s.class_name}</p>
                                    <p><strong>Issued:</strong> {issuedDate}</p>
                                </div>
                            </div>
                            <div className="absolute bottom-0 left-0 w-[280px] h-[45px] flex justify-center items-end overflow-hidden mb-3">
