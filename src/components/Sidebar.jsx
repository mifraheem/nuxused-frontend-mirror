            ...
            <nav className="flex-1 px-2 py-4">
                <ul className="space-y-2">
                    ...
                    {permissions.includes("users.view_classname") && (
                        <SidebarLink to="/admin/class-management" icon="pi pi-table" label="Class Management" currentPath={location.pathname} isSidebarOpen={isSidebarOpen} />
                    )}
                    {permissions.includes("users.view_subject") && (
                        <SidebarLink to="/admin/subject-management" icon="pi pi-book" label="Subject Management" currentPath={location.pathname} isSidebarOpen={isSidebarOpen} />
                    )}
                    ...
                    {(permissions.includes("users.view_studentattendance") || permissions.includes("users.view_staffattendance")) && (
                        <SidebarDropdown
                            ...
                            toggleTab={toggleTab}
                            items={[
                                permissions.includes("users.view_studentattendance") ? { to: "/admin/track-students-attendance", label: "Student Attendance" } : null,
                                permissions.includes("users.view_staffattendance") ? { to: "/admin/track-staff-attendance", label: "Staff Attendance" } : null
                    ...
                    {permissions.includes("users.view_gradecriteria") && (
                        <SidebarLink to="/admin/grade-criteria" icon="pi pi-chart-line" label="Grade Criteria" currentPath={location.pathname} isSidebarOpen={isSidebarOpen} />
                    )}
                    {(permissions.includes("users.view_finalresult") || permissions.includes("users.view_result")) && (
                        <SidebarDropdown
                            ...
                            toggleTab={toggleTab}
                            items={[
                                permissions.includes("users.view_studentresult") ? { to: "/admin/student-results", label: "Student Results" } : null,
                                permissions.includes("users.view_finalresult") ? { to: "/admin/final-results", label: "Final Result" } : null
                            ].filter(item => item !== null)}
                            currentPath={location.pathname}
                            isSidebarOpen={isSidebarOpen}
                        />
                    ...
                    {permissions.includes("users.view_exam") && (
                        <SidebarDropdown
                            ...
                            activeTab={activeTab}
                            toggleTab={toggleTab}
                            items={[{ to: "/admin/create-exam", label: "Create Exam" }]}
                            currentPath={location.pathname}
                            isSidebarOpen={isSidebarOpen}
                        />
                    )}
                    {permissions.includes("users.view_timetable") && (
                        <SidebarDropdown
                            ...
                            activeTab={activeTab}
                            toggleTab={toggleTab}
                            items={[{ to: "/admin/manage-timetable", label: "Manage Timetable" }]}
                            currentPath={location.pathname}
                            isSidebarOpen={isSidebarOpen}
                        />
                    )}
                    ...
                    {permissions.includes("users.change_staffprofile") && (
                        <SidebarDropdown
                            ...
                            activeTab={activeTab}
                            toggleTab={toggleTab}
                            items={[{ to: "/admin/staff-salary", label: "Generate Staff Salary" }]}
                            currentPath={location.pathname}
                            isSidebarOpen={isSidebarOpen}
                        />
                    )}
                    ...
                    {permissions.includes("users.view_facultytask") && (
                        <SidebarLink to="/admin/weekly-task-manager" icon="pi pi-briefcase" label="Weekly Tasks" currentPath={location.pathname} isSidebarOpen={isSidebarOpen} />
                    )}
                    {permissions.includes("users.view_classannouncement") && (
                        <SidebarLink to="/admin/class-announcements" icon="pi pi-bell" label="Class Announcements" currentPath={location.pathname} isSidebarOpen={isSidebarOpen} />
                    )}
...
const SidebarLink = ({ to, icon, label, currentPath, isSidebarOpen }) => (
    <li>
        ...
        >
            <i className={`${icon} ${isSidebarOpen ? 'mr-2' : 'mx-auto'}`}></i>
...
const SidebarDropdown = ({ label, icon, tabKey, activeTab, toggleTab, items, currentPath, isSidebarOpen }) => (
    <li>
        <button
            ...
            className={`flex items-center w-full px-4 py-2 hover:bg-blue-700 rounded-lg ${!isSidebarOpen ? 'cursor-default' : ''}`}
            title={!isSidebarOpen ? label : ''}
        >
            <i className={`${icon} ${isSidebarOpen ? 'mr-2' : 'mx-auto'}`}></i>
            {isSidebarOpen && <span>{label}</span>}
            ...
            <ul className="ml-6 mt-2 space-y-2">
                {items.map((item, idx) => (
                    <li key={idx}>
                        <Link
                            to={item.to}
                            className={`block px-4 py-2 rounded-lg transition-all duration-200 hover:bg-blue-700 ${currentPath === item.to ? 'bg-blue-800 font-semibold' : ''}`}
                        >
                            {item.label}
                        </Link>
                    </li>
